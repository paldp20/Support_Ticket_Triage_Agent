...
```
> Note: This will take time as sentence-transformers require installations of torch, transformers and other model files. This can be avoided by just using TF-IDF for vectorization: `SET ENV USE_TRANSFORMER=false` or install CPU-only torch: `RUN pip install --no-cache-dir torch --index-url https://download.pytorch.org/whl/cpu`. 
